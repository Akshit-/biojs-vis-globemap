...
//remove zonetooltip
//var zoneTooltip = d3.select(el).append("div").attr("class", "zoneTooltip");


infoLabel = d3.select(el).append("div").attr("class", "infoLabel");
...
d3.json(opts.worldMapJSON, function(world) { 
 ...
 // Plot the positions on the map:
    function plotMarkers(country){   
          ...
          var locationsChild = locations[0][0];
          while(locationsChild.firstChild){
            locationsChild.removeChild(locationsChild.firstChild);
          }
          ...
          circles = locations.selectAll('path')
            .data(collectionCountriesDataG, function(d) { 
                if(typeof(country) != 'undefined'){
                  ...
                  if(d.properties.name.indexOf(country.properties.name) >= 0){
...
var world = g.selectAll('path')
  ...
  .on("dblclick", function(d) {
   ...
   var clickable = false;
    ...
    if(collectionCountries.indexOf(d.properties.name) >=0 ){
